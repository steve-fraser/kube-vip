		...
		for _, port := range s.ExternalPorts {
			for _, gw := range gateways {
				...
				if gw.WANIPv6FirewallControlClient != nil {
					pinholeID, pinholeErr := gw.WANIPv6FirewallControlClient.AddPinholeCtx(ctx, "", port.Port, vip, port.Port, upnp.MapProtocolToIANA(port.Type), 3600)
					if pinholeErr == nil {
				...
				if !forwardSucessful {
					portMappingErr := gw.ConnectionClient.AddPortMapping("", port.Port, strings.ToUpper(port.Type), port.Port, vip, true, s.serviceSnapshot.Name, 3600)
